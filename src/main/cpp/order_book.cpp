...
void OrderBook::cancel_order(std::string &order_id){
    if (this->bid_order_map.count(order_id)){
        ...
        std::cout << "limit_ptr use_count: " << limit_ptr.use_count() << std::endl;
        std::cout << "order_ptr use_count: " << order_ptr.use_count() << std::endl;
        ...
        std::cout << "---\nlimit_ptr use_count: " << limit_ptr.use_count() << std::endl;
        std::cout << "order_ptr use_count: " << order_ptr.use_count() << std::endl;

        if (limit_ptr->orders.size()==0){
            // if i swap these two lines i get a seg fault --> only if i make &order_ptr and &limit_ptr 
        ...
        std::cout << "---\nlimit_ptr use_count: " << limit_ptr.use_count() << std::endl;
        std::cout << "order_ptr use_count: " << order_ptr.use_count() << std::endl;  
        this->bid_order_map.erase(order_id); // if i move this line to the top tests fail.
    ...
    if (this->ask_order_map.count(order_id)){
        ...
        auto &ask_limit_ptr = this->ask_limit_map[order_ptr->limit_price];
        
        this->ask_order_map.erase(order_id);
        if (ask_limit_ptr->orders.size()==0){
            this->ask_limit_map.erase(order_ptr->limit_price);
            this->ask_limits.erase(ask_limit_ptr);
        }
        return;
    }
}