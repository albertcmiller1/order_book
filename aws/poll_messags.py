...
def on_message(wsapp, message):
    ...
    for i, j in enumerate(arr):
        if j == "--order_id": order_id=arr[i+1]
        if j == "--limit": limit=arr[i+1]
        if j == "--shares": shares=arr[i+1]
        if j == "--buy_sell": buy_sell=arr[i+1]
        if j == "--event_time": event_time=arr[i+1]
        if j == "--entry_time": entry_time=arr[i+1]
